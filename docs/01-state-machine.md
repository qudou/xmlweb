...
另外，像 xmlweb 中内置的组件 Router、Rewrite 以及 AddHeaders 等都包含有事件 `enter` 的侦听器，它们都可以实例化为状态机的节点使用。下面是一个 Router 节点组件的使用示例：
...
你可以输入地址 `http://localhost:8080/index.html` 来测试此示例，该请求的返回值是一个简单的值为 `hello, world` 的字符串。
...
与组件 Flow 类似，组件 HTTP 也是节点组件并且它可以与子节点组件组合成一个状态机。在一个 web 服务应用中，HTTP 节点只能有一个，且只作为的顶层节点使用。组件 HTTP 包含一个静态参数 `listen`，此参数用于指明 web 应用服务的端口号，参数 `listen` 的默认值是 `8080`。
...
此 web 服务接收所有的具有 URL 模式 `/:id.html` 的 GET 请求。当有匹配的请求时，数据流先进入 Router 组件节点。在 Router 组件节点完成 URL 解码后会派发事件 `next`，然后数据流进入组件对象 index。在 Creater 组件节点完成数据的相关处理后派发事件 `next`，于是数据流最终达到组件对象 static。为了更清楚地认识这个过程，我们来看看组件 Creater 的具体构造。
...
该状态机比前面多了一个 Jump 节点组件，此节点用于过滤 `id` 值为 `index` 的请求，也就是 url 为 `/index.html` 的请求。对于这种请求，数据流将不再进入节点 `index`，而是直接进入节点 `static`。下面给出组件 Jump 的具体实现：
...
组件 Next 的 `enter` 事件的侦听器直接返回了 `next` 事件。然而 Next 组件节点不存在后继节点，所以该事件的派发将导致该状态机停机，也就是直接返回到上一层级的状态机。当数据流到达上一层级状态机后，数据流会向 Machine 组件节点的后继节点流动。也就是说，数据流最终会进入 Hello 组件节点。
